...
def main():
    subdir = os.listdir('ds-stereo-lab/')

    for dirc in subdir:
        print('loading images...')

        imgL = cv2.imread('ds-stereo-lab/' + dirc + '/imL.bmp')
        imgR = cv2.imread('ds-stereo-lab/' + dirc + '/imR.bmp')
        ...
        while True:
            ...
            if window_size == 5:
                filteredImg = Depth_Estimation(imgL, imgR, window_size,
                        f, b,False)
                cv2.imwrite('ds-stereo-lab/' + dirc + '/test_'
    ...
    for dirc in subdir:
        ...
        scores = []
        files = glob.glob('ds-stereo-lab/' + dirc + '/*.png')

        # Retrieving Ground Truth

        gt = cv2.imread('ds-stereo-lab/' + dirc + '/gt.bmp')
        ...
        for f in files:
            ...
            # Getting SSIM Score for each depth map estimated and Ground Truth
            ...
            if score >= .6:
                ...
                # Giving the depthmaps which are atleast 60% similar to ground truth

                print (f, score)

        plt.plot(scores,files)
        plt.show()
